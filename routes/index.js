    ...
    if (req.body.user) {
        ...
        User.getUsersBySearchWord(exp, (err, users) => {
            ...
            if (!users) {
                ...
                console.log(`This is my users' length : ${users.length}`)
                let display_users;
                let buttons;
        ...
        let str = req.body.search
        let exp = new RegExp(RegExp.escape(str))
        Course.getCoursesBySearchWord(exp, (err, courses) => {
            console.log(`This is my users' length : ${courses.length}`)
            if (err) throw err
            if (!courses) {
                let user = req.user
...
router.get('/search/courses/:searchWord/:page', ensureAuthenticated, (req, res) => {
    let { searchWord, page } = req.params;
    let str = searchWord
    let exp = new RegExp(RegExp.escape(str))
    Course.getCoursesBySearchWord(exp, (err, courses) => {
